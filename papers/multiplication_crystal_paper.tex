...
\medskip
\noindent\textbf{Keywords:} primes; multiplication crystal; explicit formula; diffraction;
quasicrystals; Beurling generalized primes; primon gas; expository.

\smallskip
\noindent\textbf{MSC 2020:} primary 11N05; secondary 11M06, 52C23, 11-02.
...
sought to retrofit. Yet characteristic zero retains the Frobenius's shadow: the integers carry
their canonical $\lambda$-ring structure --- commuting Frobenius lifts $\psi^p$, whose defining
congruence $\psi^p(n) \equiv n^p \pmod p$ is Fermat's little theorem --- and Borger's programme
\cite{Borger} proposes exactly this shadow as the definition of descent to the absolute point
beneath both universes. The machine verifies that the shadow is load-bearing
(\texttt{witt\_frobenius.py}): the Witt addition and multiplication laws' divisions by $p$ and
$p^2$ clear at every tested prime and depth, and the ghost components turn the crystal-blind Witt
sums into componentwise arithmetic --- what survives of the mirror's engine, still running in
ours. One dial, two absences, every mystery. The thesis, in its final compression: the
...
(\texttt{logic\_column.py}) add seconds each, as do the Witt-vector checks of Section~6
(\texttt{witt\_frobenius.py}). The weld laboratory, the recursion census, the skeleton-null scanner, and the extraction run add
a few minutes more. The companion papers' exact systems \cite{CompanionI, CompanionII,
CompanionIII} were re-used where indicated and were themselves machine-verified as described there.\par}
...
\bibitem{Borger} J. Borger, $\Lambda$-rings and the field with one element, arXiv:0906.3146
(2009).
