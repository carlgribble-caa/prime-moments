...
\author{Carl Gribble\thanks{Draft v0.1, expository companion to the research papers
\cite{CompanionI,CompanionII,CompanionIII} (same author). Developed in collaboration with Claude
(Anthropic); every numerical table in this essay was machine-verified as described in Section~7.
...
primes and prime powers of a window and at nothing else. Finally we chart its symmetries: maximally
...
a hundred). In this language the Riemann Hypothesis is a confinement statement: the diffraction
...
theorem, machine-verified here on an elliptic curve at $428$ primes --- isolating the
...
the multiplication crystal. Two features deserve immediate notice. First, the primes are not special
...
angles. Point sets produced this way are the mathematical models of \emph{quasicrystals}: aperiodic
to the eye, crystalline in origin, and recognizable by their diffraction, to which we will return. A
...
Quasicrystals are identified in the laboratory by diffraction: shine waves on an aperiodic point set,
and hidden crystalline order announces itself as sharp Bragg peaks. The multiplication crystal's
shadow can be diffracted in both directions, and both experiments succeed.
...
within hundredths. The diffraction pattern of the primes is the zeros.
...
thesis is stark: every aperiodic pathology of the integers should become an exactness. The machine's
...
\cite{SawinShusterman}. What the crystal formulation adds is operational: the conjecture names its
...
Odlyzko's, the symmetry types Katz's and Sarnak's. The mirror dictionary is Weil's
...
minute. The weld laboratory, the recursion census, the skeleton-null scanner, and the extraction run add
...
constructive routes and an obstruction theorem, companion manuscript (2026). % TODO: replace with
% arXiv ID once posted.
...
repository (2026), \texttt{https://github.com/USERNAME/REPO}. % TODO: fill in repository URL.
\end{thebibliography}
