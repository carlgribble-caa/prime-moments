...
- **RH7 (done, first light).** The overtone theory: existence earned out of sample —
  ...
  constant-persistence model refuted (the m = 9 anomaly decays toward the law with height —
  second-order scaling); clause withheld per prereg, height-scaled model queued. Window 4 is
  the transfer law's fourth out-of-sample confirmation. Instrument: `code/overtone.py`.
- **RH6 (done).** The derivation bridge: sampling excluded as mechanism by machine-checked
  lemma (in-span recovery exact); the continuous line content of S(t) measured FULL
  (0.99 ± 0.02, 15/15 cells, both families) while only the zero-position displacement
  sequence attenuates — the committed bridge gates refuted, localizing the law onto its true
  object. Conjecture restated (localized); derivation target precisely specified: the
  jump-point sampling weight, a pair-correlation object (Montgomery–Goldston), conjecturally
  GUE-universal. Instrument: `code/transfer_theory.py`.
- **RH5b (done).** The conjecture's cross-family test (falsification item iii): on
  L(s, χ₋₄) the same law with the same recorded constant holds (magnitude devs
  0.022–0.050), even frequencies silent as the character demands, sign pattern the
  character's. Mechanism clause weakened (fixed a beat the local-variance variant, E1
  refuted); law clause strengthened. Overtone pattern at prime powers recorded as the
  next refinement target. Instrument: `code/dirichlet_transfer.py`.
- **RH5 (done).** The transfer law, out of sample: the Debye–Waller law
  T = exp(−a·(ω/2πρ̄)²), a = 1.1416, fitted on two windows and confirmed at a third it never
  saw (gated deviations 0.003–0.036; beats the linear/form-factor alternative 6.6× in SSE) —
  **the lane's first formally labeled CONJECTURE**, with falsification protocol (fourth
  window; the anomalous m = 8, 9, 13; Dirichlet L-functions; derivation from explicit formula
  + pair correlation). Instrument: `code/transfer_law.py`.
- **RH4b (done).** Height dependence of the transfer, out of window: all four gated
  frequencies' attenuation weakens on zeros 141..200 (0.92 → 0.98 at ln 2) — the
  explicit-formula amplitudes are the dense-sampling limit. Instrument:
  `code/transfer_height.py`. Note 4 revised to cover the corrections thread; recompiled clean.
- **RH4 (done, first light).** The primes read directly in the Hamiltonian coefficients by
  committed matched filters: detected at every gated frequency with null off-prime controls
  (R² = 0.93), but the committed slope-1 amplitude law REFUTED — the prime-to-coefficient
  transfer attenuates monotonically in frequency; the measured profile plus a two-factor
  post-hoc diagnosis is the deliverable. Instrument: `code/coefficient_primes.py`.
...
### WP-C — The Compression Lane (standing; opened 2026-08-08)
**Goal.** "Is there a closed form for π(x)?" formalized as evaluation complexity: a
fixed-length exact expression ≈ a poly(log x)-time exact algorithm. Sibling of WP-RH: same
object, information-theoretic face. The spec matrix has exactly one OPEN cell — exact π(x)
in poly(log x), containing the parity of π(x) as its weakest unsolved form — flanked by
theorems (AKS local primality; x^{1/2+ε}/x^{2/3} counting), the programme's own
kernel-checked bit floor (WP7.H2), and measured incompressibility profiles. All
"incompressible" claims are per observer class; compressors are entropy proxies, not
Kolmogorov estimators.
**Stages.**
- **C1 (done, first light).** The spec matrix + the parity-stream test + the two-skeleton
  conjecture at the compressor class. Instrument: `code/compression_question.py`.
- **C2 (done).** The trained observer, handed the local congruence skeleton, extracts the
  computed divisibility content zlib missed (+0.185 nats vs 0.166 predicted) and sequence
...
- **C3b (done).** The micronat bound gated out of sample on windows no instrument had
  touched ([1e8, 2e8), [2e8, 3e8)): excesses 0.9 and 1.2 micronats vs noise floors of 1.8 —
  below pure-noise expectation. The two-skeleton conjecture at pair level at MACHINE status;
  WP6's fifth measured base camp. Instrument: `code/excess_gate.py`.
- **C4 (candidate).** The WP-C librarian pass; the WP6 write-up over the five base camps.
...
| WP10 | rolling |
| WP-C | **C1 + C2 + C3 + C3b done** 2026-08-08. **C3b** under prereg `9e43a05` (`code/excess_gate.py`, in run_all, 43 s, both gates PASS, all predictions confirmed): the beyond-skeleton pairwise excess gated OUT OF SAMPLE on untouched windows [1e8, 2e8) and [2e8, 3e8) — **0.9 and 1.2 micronats vs noise floors of 1.8** (ratios 0.53/0.69, below pure-noise expectation): the congruence model accounts for the pair structure of ~5M primes per window to statistical zero; two-skeleton at pair level at MACHINE status, WP6 base camp five. **C3** under prereg `70ab30d`: HL verified 0.9992–1.0002 at ten gaps; wheel-conditional enhancements constant to 0.08% with mod-7 jumps 1.200/1.201 vs predicted 1.20; raw-MI gate refuted on a diagnosed design flaw (skeleton-induced MI conflated), decomposition: model 0.006994 of 0.006950 nats, excess 3 micronats. **C1 + C2**. **C2** under prereg `e482ace` (`code/compression_observers.py`, in run_all, 39 s, all gates PASS, all predictions confirmed): the congruence-informed trained observer extracts **+0.1846 nats** of divisibility content (computed prediction 0.166 — N1 confirmed) that zlib had missed; with the skeleton-enriched baseline (current + 3 preceding positions' flags, making HL-derivable structure representable), **24-bit history + densities add +0.00003 nats — nothing** (M2, both model classes): the two-skeleton conjecture now holds at three measured observer strata (compressor/C1, feature/WP9c, trained-sequence/C2); parity stream coin-like at heights 10^6..10^8 (ratio 1.0000); entropy ladder max excess 0.0022 bits. HL-resolution caveat recorded (below ~0.001-nat holdout resolution — C3 candidate). **C1** —  first light under prereg `f6f2e66` (`code/compression_question.py`, in run_all, 3 s): **all five gates PASS on first run, all three predictions confirmed** — the parity stream of π(x) at 8192 geometric points compresses to the byte identically to a coin control (ratio 1.0000); the best of zlib/bz2/lzma captures the indicator's wheel skeleton (0.297 < 0.309 bits/pos) and finds nothing beyond it (wheel-restricted residual at 0.9947 of a density-matched control) — **the two-skeleton conjecture confirmed at the compressor observer class**, a new base-camp row for WP6; local primality demonstrated polylog (MR = sieve, 200 integers) sharpening the OPEN cell to global aggregation; the spec matrix printed with exactly one OPEN cell (exact π(x) in poly(log x)). Obstruction profile, not impossibility proof, and labeled so |
| WP-RH | **RH1–RH7 done; CONJECTURE stated, cross-family confirmed, localized; overtone existence earned; Note 4 revised & compile-verified (RH6/RH7 revision queued)** 2026-08-07. **RH7** under prereg `af03b78` (`code/overtone.py`, in run_all, 105 s): Landau input verified at the percent level over 500 zeros (ratios 0.97–1.00, five lines); **H3 PASS — the sharpest out-of-sample hit of the lane: m = 25 and m = 27, never measured anywhere, suppressed below the DW law (pooled −0.339, −0.176)** with genuine-prime controls conforming (H4: +0.035, −0.010); committed persistence model REFUTED (H2: window-4 m = 9 at 0.647 vs band [0.22, 0.55] — the anomaly decays toward the law with height, 0.37/0.39/0.46/0.65, the second-order scaling); clause withheld per prereg, existence stands at MACHINE, height-scaled overtone model queued with the jump-point weight as the lane's two derivation targets. P1: window 4 = the transfer law's **fourth consecutive out-of-sample confirmation** (mean |dev| 0.024). **RH6** under prereg `8016883` (`code/transfer_theory.py`, in run_all, 230 s): the derivation bridge — (A) machine-checked lemma: displaced-lattice sampling cannot attenuate in-span lines (single-line 1.0000; nine-line prime-like field 1.000×9 after a bring-up fix: the prereg's amplitude scale was non-contractive, Σαν ≈ 1.6, first run's fixed point never converged — corrected to 0.15 with a loud assert, recorded); (B/C) **bridge gates G-B1/G-C1 REFUTED, and the refutation is the localization**: the continuous line content of S(t) = N − N̄ is FULL (0.99 ± 0.02 in all 15 cells, ζ three windows + beta; fitted continuous a ≈ 0: no attenuation exists in continuous time, P2 refuted decisively) while only the zero-sampled displacement sequence carries the DW law. Conjecture restated (localized): primes enter S(t) at full amplitude at all heights; the DW attenuation is the price of reading them from the zeros' POSITIONS — exactly the data the Hamiltonian reconstruction consumes, so the law remains the OPEN cell's fingerprint; precise derivation target: the jump-point sampling weight (Montgomery–Goldston pair-correlation object), whose GUE universality would explain the family-independent a ≈ 1.14. Exit-code semantics per RH1/RH4 precedent. **RH5b** under prereg `c00a255` (`code/dirichlet_transfer.py`, in run_all, 150 s, gates D0–D3 all PASS): 150 zeros of Dirichlet beta located (heights 6.0–234.4, indexing self-verified at 0.503 < 0.9); **the transfer law transfers**: same recorded a = 1.1416 gives magnitude deviations 0.022/0.026/0.050 at m = 3/5/7, the even frequencies are forced-null and measure silent (m = 2 at 0.001), and the sign pattern is exactly χ₋₄'s (−,+,−,−,+) — the law is a property of the prime–zero coupling, not of ζ alone. E1 refuted-and-recorded: the fixed constant beat the local-variance mechanism variant (a₃ = 0.680), so the mechanism clause (a ≈ 2π²Var S̄) is weakened while the law clause strengthens; E2 confirmed (ζ's m = 13 anomaly is not family-persistent); **overtone pattern recorded unclaimed**: every anomalous frequency to date is a harmonic of a strong prime line (ln p^k = k ln p), pointing at a second-order transfer theory as the next prereg. **RH5** under prereg `25e5e9e` (`code/transfer_law.py`, in run_all, 77 s, gates S1 + S2 PASS, Q1 + Q3 confirmed, Q2 refuted-and-recorded): two candidate transfer laws fitted on windows 1–2 and recorded in the prereg (DW: a = 1.1416; linear: c = 0.5255 ≈ 1/2), then tested at zeros 301..400 (heights 544–680) which neither had seen — **the Debye–Waller law holds out of sample** (gated |dev| 0.003/0.004/0.036/0.004; SSE 6.6× better than linear; zero-parameter mechanism variant a = 2π²⟨u²⟩ within 0.10). The lane states its first labeled **CONJECTURE** (claims-taxonomy state iii, first use in the programme): T(ω; W) = exp(−a(ω/2πρ̄_W)²), a ≈ 2π² Var(S̄) — the transfer is the Debye–Waller factor of the zeros' displacement field, explicit-formula amplitudes its τ → 0 limit — with falsification protocol: fourth window, the anomalous frequencies m = 8/9/13 (deviations 0.25/0.29/0.15, recorded under Q2's refutation), Dirichlet L-functions, and derivation from explicit formula + pair-correlation input (the partial-theorem target). Provenance chain RH4 → RH4b → RH5: three preregistered instruments, three disjoint zero sets. **RH4b** under prereg `49b66b5` (`code/transfer_height.py`, in run_all, 27 s, gates Z1 + Z2 PASS, V1 + V2 confirmed): out-of-window test on zeros 141..200 (heights 305–396, disjoint from all prior analyses) — the attenuation weakens with height at **4/4 gated frequencies** (0.922 → 0.984 at ln 2; mean improvement +0.128 in the committed band): the transfer shape is a height-dependent sampling effect and **the explicit-formula amplitudes are its dense-sampling limit**; spec constraint 1 in final measured form (attenuates with ω/ρ̄, approaches Λ-amplitudes as the spectrum densifies). One prereg-level sign error (field convention, RH2-Abel precedent) caught by the frozen gates on first run, corrected with thresholds unchanged, first-run table recorded. Note 4 gains the corrections-thread section (RH3c/RH4/RH4b); recompiles clean, cite check 33/33. **RH4** under prereg `26f66ab` (`code/coefficient_primes.py`, in run_all, 8 s): matched-filter decomposition of the coefficient residual over finite-difference templates at prime-power frequencies — **every gated frequency detected, sign +, all four off-prime controls null, R² = 0.93**, linearity 7e-7; but headline gate X2 (slope-1 explicit-formula amplitudes) **REFUTED on physics**: monotone attenuation 0.64 (ln 2) → 0.31 (ln 7). Post-hoc diagnostic separates two mechanisms — RvM premise attenuation even at zero-sampling (0.92 → 0.33) and smooth-coordinate Debye–Waller-type decoherence (reproducing the instrument's ratios) — recorded with numbers in the results file; amplitude law OPEN, detection MACHINE. Spec constraint 1 sharpened: candidates must match the frequency-decaying transfer shape, not just the comb's presence. Exit-code semantics per RH1 precedent (instrument gates X1/X3 gate CI; X2 prints REFUTED permanently). **RH3c** under prereg `abb3d39` (`code/plateau_bias.py`, in run_all, 5 s, gates V1 + V2 PASS, W1 + W2 confirmed): the smooth comb — same mean density, no arithmetic — reproduces the plateau bias cell by cell (max difference 0.221 vs gate 1.0): **the bias is a density effect**; combined ledger: gap, window, universal plateau, and its deviation are all smooth-derivable — the arithmetic content of the canonical-system data is confined to the rigidity-scale residual fluctuations. Next thread recorded (unclaimed): Fourier content of the coefficient residual vs the prime frequencies log p — RH2's B5 one level down. **RH3b** under prereg `ee3484d` (`code/two_interval_plateau.py`, in run_all, 4 s, gate T1 PASS): the two-interval law predicted plateaus it had never seen — N = 50: −1.07/−1.75, N = 60: −1.43/−2.79 against ±3, all negative (U1) with odd tighter than even (U2) as committed → the plateau claim earned at MACHINE status; the growing negative bias recorded as observed-but-unclaimed (density-correction prereg candidate). The RH3→RH3b pair (refuted committed window → post-hoc reading refused → out-of-sample prereg → confirmation) recorded as the lane's template for promoting observations to claims. **Note 4** (`papers/positivity_lane.tex`): charter + honest pricing, the three instruments with their refutations as first-class content, the spec matrix, related-work/non-claims, AI disclosure; compiles clean (pdflatex ×2, zero warnings), cite cross-check 33/33 both directions. **RH3** under prereg `8675814` (`code/requirements_spec.py`, in run_all, 9 s, all six gates PASS): the spec matrix printed and complete with exactly one OPEN cell — (self-adjoint realization, zeta). New machine checks: RvM density over 100 zeros (max dev 0.4979, P2 confirmed); rigidity ordering picket < zeros (**var 0.1253**, GUE ~ 0.18, P1 confirmed) < pi-digit Poisson (1.0755); Petersen spectrum↔geodesics via Ihara–Bass exact to 6e-29 for m ≤ 12; the smooth split tr(A^m) = 10·t_m below the girth with **first excess 120 = the pentagons traversed** (the graph miniature of RH2's residual-is-the-primes); fresh EC trace formula at p = 13 over F_169 (180 = 180). Mining redirect recorded in prereg (Jacobi tail limits are truncation-set — PSLQ deferred); P3 tail-identification REFUTED by window choice (committed window sat in the truncation-decay regime; post-hoc plateau n = 11..40 matches the two-interval equilibrium to 1.4 but stays unclaimed) — net: the "tail forgets arithmetic" constraint strengthened. Constraint lines pinned on the OPEN cell: not smooth (prime comb required at fluctuation order) and tight (zero margin in every solved universe). First-light phase of the lane complete. **RH2** under prereg `72d1494` (`code/canonical_system.py`, in run_all, 55 s): (A) Jacobi/Krein data of the zero comb by Stieltjes at dps 40 — symmetry to 9e-37, measure re-derived from J_80 to 1.3e-38; rigidity in Hamiltonian coordinates: std(zeros−smooth)/std(Poisson−smooth) = **0.246** over n = 6..70 (Q2 confirmed; Poisson control deterministic from pi's digits); (B) arithmetic-free canonical object: closed-form Abel-inverted potential (defining property WKB == Nbar+1/8 verified to 5e-5), Numerov spectrum counts exactly 60 below the window, RMS tracking 0.537, and the **headline: detrended residual vs damped prime sum r = 0.9999** — Q1's committed band [0.6, 0.95] refuted *from above*: delta*rho ~ 1/8 − S is numerically an identity, so the smooth Hamiltonian misses exactly the primes and nothing else (no circularity: E_k sees no primes/zeros); Maslov offset measured +0.1251 vs predicted 1/8 (Q3); (C) Frobenius measure reconstruction terminates at step 2, residual 0 — finite exact Hamiltonian where the RH-analogue is a theorem (Q4). Two bring-up bugs (Abel 1/pi prefactor; silent bracket saturation producing constant delta = cap) found by gates B3/B4, fixed with thresholds unchanged, fully recorded in the results file. Classification: known-theorem-derivable mechanism (Wu–Sprung / RvM+WKB); claimed: measured tables, controls, discipline. Requirements spec for RH3 opened (results §Carried forward). **RH1** —  first light under prereg `8252b9a` (`code/weil_positivity.py`, in run_all, 34 s): Gram matrix of the Weil form on 12 even Gaussian profiles assembled from WP3-engine evaluations at 23 grid centres — identity geo-vs-zero to 2.6e-23 across all 78 entries, positivity at lambda_min = -8.5e-24; prime-free window (supp g in (-log2, log2), prime term identically zero) positive at 9.1e-4 with zero-side agreement 4.2e-6 matching the committed tail estimate; EC calibration (y² = x³+x+1 / F_10007, a = -57): Frobenius Toeplitz form PSD of exact rank 2 (~1e-40 floor), Hasse-violating control fires at -38.96 with golden-ratio growth. **Prediction B3 REFUTED and recorded** (registry): the near-kernel is not spanned by gap profiles — measured mechanism: resolved rank of M = 6 = zeros engaged; on a finite spectral window the Weil form is a finite-rank sampler (known-theorem-derivable; kept as calibration, not claimed as novel). Diagonal gap ladder confirmed as committed (rungs 3e-43 → 0.2). Next: RH2 (inverse spectral reconstruction against solved-universe Hamiltonians) |
