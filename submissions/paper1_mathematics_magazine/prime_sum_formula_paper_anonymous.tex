...
\author{[Author details blinded for peer review]\thanks{Version 1.0, August 2026. Companion to \emph{Certified exact sums of primes over dyadic
...
numbers, and the primes are exactly the integers the table cannot make. (The table complement is a
classical device --- Sundaram's sieve \cite{Sundaram} generates the primes exactly this way --- and the
\emph{de-duplicated} table, the set of distinct products, is the subject of the Erd\H{o}s
multiplication table problem \cite{ErdosMT,Ford}; our concern below is with the table's exact
multiplicities rather than its cardinality.) An arithmetic amateur's very
...
(i) For a target interval $(A, B]$ of values, every layer is summed over the hyperbola
shell\footnote{``Shell'' here means a product level region, the difference of two $k$-dimensional
Dirichlet hyperbola regions --- not the hyperbolic shells (level sets of indefinite quadratic forms)
of the lattice-point literature.}
...
$O(\sqrt B)$ points (the quotient set alone has cardinality $2\sqrt B + O(1)$, exactly determined
in \cite{Heyman}; e.g.\ $642$ points for $B = 10^5$).
\end{proposition}
...
system's cost profile feels primality before its arithmetic announces it. The underlying object has a
literature of its own: the floor-quotient set $\{\fl{y/d}\}$ carries a partial-order structure studied
by Lagarias and Richman \cite{LR}, its cardinality is exactly determined \cite{Heyman}, and the primes
\emph{inside} it are actively studied \cite{HeymanP,MaWu}. The observable here is of a different kind:
the bare cardinality of the quotient set grows at every step and does not discriminate primes, whereas
the growth of the closed evaluation lattice is supported exactly on the divisors of $y+1$ --- kin to
the folklore reading of $D(n) - D(n-1) = d(n)$ for the divisor summatory function, minimal exactly at
the primes.
\end{remark}
...
\cite{McKenzie} (unrefereed manuscripts, whose complexity analysis is McKenzie's own); within the same
lineage, Sladkey's $O(x^{1/3})$ divisor-summatory algorithm \cite{Sladkey} is the natural subroutine
for pressing below the $x^{2/3}$ class. The $j=0$ extraction is Riemann's inversion
\cite{Riemann,Edwards}; Legendre's sieve is the
dimension-parity twin; the combinatorial prime-counting lineage
\cite{DR,Staple} and its prime-power-sum extensions \cite{Orlov,Lucy,Walisch} contain algorithms of better
exponent ($x^{2/3}$-class) than our $x^{3/4}$-class implementation, with record prime-sum computations
to $10^{26}$ \cite{Walisch}; and an elementary route of Hirsch, Kessler, and Mendlovic reaches
$\widetilde{O}(\sqrt x)$ for counts and sums alike \cite{HKM}. The companion paper \cite{Companion} develops a
...
AI system contributed substantially to the mathematical development and proofs, wrote the
verification code, drafted and revised the manuscript text, and assisted with literature search; the
reason for its use was to enable a single non-specialist author to develop, check, and document the
material to a publishable standard. Every numerical and mathematical claim was subsequently machine-verified by the
...
\texttt{projecteuler.net/thread=10;page=5\#111677}.
\bibitem{HW} G.~H. Hardy and E.~M. Wright, \emph{An Introduction to the Theory of Numbers}, 6th ed.,
Oxford Univ. Press, 2008.
\bibitem{Sundaram} V. Ramaswami Aiyar, Sundaram's sieve for prime numbers, \emph{The Mathematics
Student} \textbf{2} (1934), 73.
\bibitem{ErdosMT} P. Erd\H{o}s, Some remarks on number theory (Hebrew, English summary),
\emph{Riveon Lematematika} \textbf{9} (1955), 45--48.
\bibitem{Ford} K. Ford, The distribution of integers with a divisor in a given interval,
...
\bibitem{Sladkey} R. Sladkey, A successive approximation algorithm for computing the divisor
summatory function, arXiv:1206.3369 (2012).
\bibitem{HKM} D. Hirsch, I. Kessler, and U. Mendlovic, Computing $\pi(N)$: an elementary approach in
$\widetilde{O}(\sqrt{N})$ time, arXiv:2212.09857 (2022).
\bibitem{Walisch} K. Walisch and D. Baugh, \emph{primesum} (software),
\texttt{github.com/kimwalisch/primesum}.
\bibitem{LR} J.~C. Lagarias and D.~H. Richman, The floor quotient partial order, \emph{Adv. in Appl.
Math.} \textbf{153} (2024); arXiv:2212.11689.
\bibitem{Heyman} R. Heyman, Cardinality of a floor function set, \emph{Integers} \textbf{19} (2019),
\#A67.
\bibitem{HeymanP} R. Heyman, Primes in floor function sets, arXiv:2111.00408 (2021).
\bibitem{MaWu} R. Ma and J. Wu, On the primes in floor function sets, \emph{Bull. Aust. Math. Soc.}
\textbf{108} (2023), 236--243; arXiv:2112.12426.
\bibitem{Companion} [Author blinded], Certified exact sums of primes over dyadic intervals via a
ladder of generalized factorials, companion manuscript (2026); preprint DOI blinded for peer review.
\bibitem{Code} [Author blinded], Reference implementations accompanying this paper and \cite{Companion},
code repository (2026); repository URL blinded for peer review, available to the editors on request.
\end{thebibliography}
