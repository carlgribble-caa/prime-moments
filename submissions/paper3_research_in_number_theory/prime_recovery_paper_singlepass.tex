...
\author{Carl Gribble\thanks{Version 1.0, August 2026. Companion to \emph{Prime counts and prime
...
\begin{document}
% Reference data pre-resolved so this file renders correctly in a single
% LaTeX pass (submission-system builds). Values match the .aux of a full run.
\makeatletter
\expandafter\gdef\csname r@eq:intch\endcsname {{1}{2}{}{equation.1}{}}
\expandafter\gdef\csname r@eq:wch\endcsname {{2}{2}{}{equation.2}{}}
\expandafter\gdef\csname r@thm:int\endcsname {{1}{3}{}{theorem.1}{}}
\expandafter\gdef\csname r@thm:prod\endcsname {{2}{3}{}{theorem.2}{}}
\expandafter\gdef\csname r@rem:prony\endcsname {{1}{4}{}{remark.1}{}}
\expandafter\gdef\csname r@def:scheme\endcsname {{1}{4}{}{definition.1}{}}
\expandafter\gdef\csname r@thm:lb\endcsname {{3}{5}{}{theorem.3}{}}
\expandafter\gdef\csname b@CompanionI\endcsname {1}
\expandafter\gdef\csname b@CompanionII\endcsname {2}
\expandafter\gdef\csname b@Linnik\endcsname {3}
\expandafter\gdef\csname b@McKenzie\endcsname {4}
\expandafter\gdef\csname b@Cheb\endcsname {5}
\expandafter\gdef\csname b@Macdonald\endcsname {6}
\expandafter\gdef\csname b@Prony\endcsname {7}
\expandafter\gdef\csname b@Mills\endcsname {8}
\expandafter\gdef\csname b@Willans\endcsname {9}
\expandafter\gdef\csname b@Erdos\endcsname {10}
\expandafter\gdef\csname b@Polymath\endcsname {11}
\expandafter\gdef\csname b@MSW\endcsname {12}
\expandafter\gdef\csname b@BOT\endcsname {13}
\expandafter\gdef\csname b@VandenEynden\endcsname {14}
\expandafter\gdef\csname b@Golomb\endcsname {15}
\expandafter\gdef\csname b@Fridman\endcsname {16}
\expandafter\gdef\csname b@JSWW\endcsname {17}
\expandafter\gdef\csname b@PrunescuShunia\endcsname {18}
\expandafter\gdef\csname b@Wilf\endcsname {19}
\expandafter\gdef\csname b@Dudley\endcsname {20}
\expandafter\gdef\csname b@CaldwellCheng\endcsname {21}
\expandafter\gdef\csname b@KolpakovRocke\endcsname {22}
\expandafter\gdef\csname b@Kontoyiannis\endcsname {23}
\expandafter\gdef\csname b@BrodnikMunro\endcsname {24}
\expandafter\gdef\csname b@ErdosRenyi\endcsname {25}
\expandafter\gdef\csname b@Moitra\endcsname {26}
\expandafter\gdef\csname b@BY\endcsname {27}
\expandafter\gdef\csname b@Code\endcsname {28}
\makeatother
...
\medskip
\noindent\textbf{Keywords:} prime recovery; moment problems; Newton's identities; elementary
symmetric functions; Chebyshev's functional; Prony's method; information-theoretic lower bounds.

\smallskip
\noindent\textbf{MSC 2020:} primary 11N05; secondary 05E05, 11Y16, 94A17.
...
Recovery of a finite multiset from its power sums is classical and has a modern general theory
\cite{MSW}; in exact arithmetic, the same decoder --- power sums, a Hankel/Newton step, integer
roots --- is the sparse-interpolation algorithm of Ben-Or and Tiwari \cite{BOT}. What is specific
here is the provenance and the support: the power sums arrive from a prime-free channel, and the
roots are the window primes.
...
The kernel of Route II --- the primorial determines the primes --- has classical avatars. Gandhi's
formula recovers the next prime from the divisors of $p_n\#$ by a M\"obius sum
\cite{VandenEynden,Golomb}, and Fridman, Garbulsky, Glecer, Grime, and Tron Florentin exhibited a
single real constant, in effect a primorial-denominated series, whose floor recurrence emits every
prime \cite{Fridman}. Both are decoders whose data store the answers, and both therefore live outside
the model of Section~5, exactly as Mills's constant does; Theorem~\ref{thm:prod} differs in taking
the real number from a computable moment channel and pricing the window's information in delivered
bits.
...
estimates using the gap $|p_i - p_j| \ge 2|i - j|$ between distinct odd primes, but are tedious;
sharp forms exist in the super-resolution literature, where the required precision is governed by
node separation \cite{Moitra,BY}, and the primes' gap $\ge 2$ places the window in the
well-separated regime. We
...
rigorous kernel of the no-closed-form folklore (classically articulated by Wilf and Dudley
\cite{Wilf,Dudley}), concerns schemes whose decoders learn about the window
...
The counting step is the standard information-theoretic floor for representing $r$-subsets of an
$m$-universe, as in succinct data structures \cite{BrodnikMunro} and combinatorial search
\cite{ErdosRenyi}; the content here is its application to the moment channels and the accounting of
Section~7. We note also that ``entropy of the primes'' has an established different use ---
entropy \emph{proofs} of Chebyshev-type estimates \cite{Kontoyiannis} --- distinct from the
subset-counting entropy priced here; and that the nearest prior impossibility statement we know, an
algorithmic-information argument against ``formulas for primes'' \cite{KolpakovRocke}, is heuristic
and asymptotic in character, where Theorem~\ref{thm:lb} is a finite, unconditional count in an
explicitly delimited channel model.
...
stores the answers --- computing the constant requires knowing the primes it emits
\cite{CaldwellCheng}; Willans's formula \cite{Willans} passes unbounded information through the
evaluation of factorials under a floor, as do its Diophantine and arithmetic-term descendants
\cite{JSWW,PrunescuShunia}; a sieve is a decoder that ignores the channel and recomputes
from $m$. All of these are outside Definition~\ref{def:scheme}, and their exclusion is precisely what
...
$\sqrt{x}$-barrier literature \cite{Polymath} is the state of the art. That work is also, in the
present vocabulary, a moment channel with a partial decoder: it extracts one exact bit of the window
--- the parity of the number of its primes --- in time $O(N^{1/2 - c})$, without enumerating them.
...
The complete code underlying this study, including the script that reproduces every recovery
experiment reported in Section~6 (\texttt{prime\_recovery.py}, driven by \texttt{run\_all.py}), is
openly available in the repository \cite{Code} under the MIT license. No external datasets were
used; all results are generated deterministically by the published code.
...
5, accessed through the claude.ai interface, 2026). The direction of the work --- the research
programme of exact moment systems for interval primes --- together
...
self-similar formula system, companion manuscript, Zenodo (2026), \texttt{doi:10.5281/zenodo.21769102}.
\bibitem{CompanionII} C. Gribble, Certified exact sums of primes over dyadic intervals via a ladder
of generalized factorials, companion manuscript, Zenodo (2026), \texttt{doi:10.5281/zenodo.21769104}.
\bibitem{Linnik} Yu.~V. Linnik, \emph{The Dispersion Method in Binary Additive Problems},
Translations of Mathematical Monographs 4, Amer. Math. Soc., Providence, 1963.
...
\bibitem{Prony} R. de Prony, Essai exp\'erimental et analytique: sur les lois de la dilatabilit\'e
des fluides \'elastiques et sur celles de la force expansive de la vapeur de l'eau et de la vapeur de
l'alkool, \`a diff\'erentes temp\'eratures, \emph{J. \'Ecole Polytechnique} \textbf{1}, cahier 2
(1795), 24--76.
...
\textbf{81} (2012), 1233--1246; arXiv:1009.3956.
\bibitem{MSW} H. Mel\'anov\'a, B. Sturmfels, and R. Winter, Recovery from power sums,
\emph{Exp. Math.} \textbf{33} (2024), 225--234; arXiv:2106.13981.
\bibitem{BOT} M. Ben-Or and P. Tiwari, A deterministic algorithm for sparse multivariate polynomial
interpolation, in \emph{Proc. 20th ACM Symp. Theory of Computing (STOC)}, 1988, 301--309.
\bibitem{VandenEynden} C. Vanden Eynden, A proof of Gandhi's formula for the $n$th prime,
\emph{Amer. Math. Monthly} \textbf{79} (1972).
\bibitem{Golomb} S.~W. Golomb, Formulas for the next prime, \emph{Pacific J. Math.} \textbf{63}
(1976), 401--404.
\bibitem{Fridman} D. Fridman, J. Garbulsky, B. Glecer, J. Grime, and M. Tron Florentin, A
prime-representing constant, \emph{Amer. Math. Monthly} \textbf{126} (2019), 70--73.
\bibitem{JSWW} J.~P. Jones, D. Sato, H. Wada, and D. Wiens, Diophantine representation of the set of
prime numbers, \emph{Amer. Math. Monthly} \textbf{83} (1976), 449--464.
\bibitem{PrunescuShunia} M. Prunescu and J.~M. Shunia, On arithmetic terms expressing the
prime-counting function and the $n$-th prime, arXiv:2412.14594 (2024).
\bibitem{Wilf} H.~S. Wilf, What is an answer?, \emph{Amer. Math. Monthly} \textbf{89} (1982),
289--292.
\bibitem{Dudley} U. Dudley, Formulas for primes, \emph{Math. Mag.} \textbf{56} (1983), 17--22.
\bibitem{CaldwellCheng} C.~K. Caldwell and Y. Cheng, Determining Mills' constant and a note on
Honaker's problem, \emph{J. Integer Seq.} \textbf{8} (2005), art.~05.4.1.
\bibitem{KolpakovRocke} A. Kolpakov and A. Rocke, On the impossibility of discovering a formula for
primes using AI, arXiv:2308.10817 (2023).
\bibitem{Kontoyiannis} I. Kontoyiannis, Some information-theoretic computations related to the
distribution of prime numbers, arXiv:0710.4076 (2007); expository version, Counting the primes using
entropy, \emph{IEEE Information Theory Workshop} (2008).
\bibitem{BrodnikMunro} A. Brodnik and J.~I. Munro, Membership in constant time and almost-minimum
space, \emph{SIAM J. Comput.} \textbf{28} (1999), 1627--1640.
\bibitem{ErdosRenyi} P. Erd\H{o}s and A. R\'enyi, On two problems of information theory,
\emph{Publ. Math. Inst. Hungar. Acad. Sci.} \textbf{8} (1963), 229--243.
\bibitem{Moitra} A. Moitra, Super-resolution, extremal functions and the condition number of
Vandermonde matrices, in \emph{Proc. 47th ACM Symp. Theory of Computing (STOC)}, 2015, 821--830.
\bibitem{BY} D. Batenkov and Y. Yomdin, On the accuracy of solving confluent Prony systems,
\emph{SIAM J. Appl. Math.} \textbf{73} (2013), 134--154.
